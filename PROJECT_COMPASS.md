...
status: active
version: 26
last_updated_utc: 2026-07-20T22:30:03Z
last_updated_by: Codex
project_owner: User
current_project_phase: Phase 5A bounded Target Position definitions/manual previews implemented over central Schema v6; Target Position, Asset State and Capital Allocation remain disabled/unconsumed, Portfolio Accounting remains in-memory and execution remains declaration-only
...
last_verified_commit_or_working_tree_state: Phase 1–5A verified Git release checkpoint in the commit containing this record, including approved bounded Target Position manual research, central Schema v6 and audited GUI/Run history; package version 0.1.0; full suite 375 passed with one upstream warning, architecture 45 passed, compileall and pip checks passed; real v5→v6 backup/migration preserved 215340 Market Bars and 365 Fetch History rows with integrity/FK checks clean and zero default Target Position rows; no account/order execution; Live and automatic submission disabled
```
...
|---|---|---|
| Primary desktop launcher | Implemented and verified | three independent GUI targets plus trusted direct shortcuts to all fifteen existing Algorithm Control core pages, including Capital Allocation, Asset State, Target Position and Run History; no feature logic in launcher |
...
| Central SQLite algorithm evidence | Implemented and verified for local research | Schema v6, verified v1→v2, v2→v3, v3→v4, v4→v5 and v5→v6 backup/migration paths, Run/Factor/Decision/Risk/Capital/Asset State/Target Position adapters and reload tests; ADR-0009/0016/0017/0019/0020/0021 |
...
Phase 5A verification adds immutable bounded curve validation, exact Decimal clamp/interpolation, deterministic repeated numeric results, structured target/difference traces, durable invalid/failed attempts, transactional Store cross-object validation, v5→v6 backup/rollback/restart, Target Position Run artifacts, GUI/chart/Launcher and dependency boundaries. The real central database is Schema v6; its verified v5 backup and migrated copy both pass integrity/foreign-key checks while preserving 215,340 Market Bars and 365 Fetch History rows. No default Target Position definition/result, network/account/order, Paper or Live path was invoked. The Launcher exposes three applications and fifteen core shortcuts. Final automated counts are recorded in `EDIT-20260720-006`.

## B2. Current Architecture

`quant_trading.factors` owns the public definition, calculation, Factor-history and exact visualization-evidence query meaning. `quant_trading.decision` owns condition/sizing trace meaning and Decision-history query meaning. `quant_trading.capital_allocation` separately owns explicit research plans and conservation; `quant_trading.asset_state` separately owns symbolic graphs, manual cycles/transitions and replay meaning. Neither supplies factual Portfolio Accounting or has an automatic consumer. `quant_trading.algorithm_control` owns editors/presentation and delegates owner rules to injected typed services. `quant_trading.run_history` owns neutral lifecycle/query contracts; independently injected central SQLite adapters store linked Factor/Decision/Risk/Capital/State evidence.
...
| `quant_trading.run_history` | neutral NO EXECUTION Run identity, lifecycle, ordered stages, bindings, messages and typed query views | formulas, domain-result semantics, SQL, GUI, accounting, execution | Implemented for local research previews |
| `quant_trading.persistence` | shared SQLite Schema v6/migration plus Run lifecycle, immutable Factor/Decision/Risk/Capital/Asset State/Target Position evidence and typed research-query adapters | formulas/rules/capital/state/target meaning, availability policy, GUI, accounting, broker/execution | Implemented; active only for explicit local research |
...
| `quant_trading.target_position` | explicit immutable curve + manual scalar/USD inputs → bounded target fraction/value/difference and structured Decimal trace | standardized state, automatic inputs, hysteresis, TradeIntent, Decision/Risk/Backtesting/Accounting/Execution | Implemented and verified; definitions/results inactive and unconsumed |
...
- Phase 5A Target Position is explicit manual research calculation. No definition is Active or consumed; standardized reference/risk state, automatic Factor/Asset State inputs, Capital/Accounting adapters, symbol authority, hysteresis/stateful levels, TradeIntent conversion and Risk/Backtesting/Execution integration remain unimplemented.
...
- Phase 1 unified Run History, Phase 2A Factor/Decision inspection, Phase 2B exact Factor visualization/export, Phase 3A research capital conservation, Phase 4A manual Asset State history and Phase 5A bounded manual Target Position are approved and implemented for explicit local research. Central SQLite is Schema v6. No production Factor calculation, numerical Risk, accounting persistence or execution has been approved or activated.
- PROPOSAL-014 is complete; no further development slice is currently approved. Later work such as standardized-state formulas, automatic input adapters, hysteresis, Target Position-to-Decision conversion, numerical Risk, full Backtesting integration and accounting persistence requires separate scoped approval and must reuse the existing Run/history contracts.

...
def test_v7_to_current_migration_backs_up_preserves_and_creates_no_links(tmp_path: Path):
    database = tmp_path / "central.sqlite3"
    ...
    assert ".schema-v7-to-v13." in backup_files[0].name
    ...
    with sqlite3.connect(database) as connection:
        ...
        assert connection.execute(
            "SELECT COUNT(*) FROM target_adjustment_decision_results"
        ).fetchone()[0] == 0

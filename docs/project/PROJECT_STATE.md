...
- Central SQLite Schema v14 reuses `runtime/data/market_history.sqlite3` for independent Market/Run/Factor/P23-1/Decision/Risk/Capital/State/Target evidence. Verified additive v1→v14 migrations preserve earlier evidence. The v13→v14 migration preserved every prior table row count, expanded the required logical schema from 74 to 94 tables, passed integrity/foreign-key checks and backfilled no P23-1 result. Immutable locked R1 v1.0.0 and v1.1.0 definitions are registered; one explicit AAPL v1.1.0 operation is stored and reloadable. No production consumer is active.
...
- P23-1 Spectral Volatility Research through P23-1E-A is `IMPLEMENTED_VERIFIED_DISABLED`: explicit XNYS/Daily/raw/split/corporate-action evidence feeds the pure three-window OLS/MAD/Welch/full-window diagnostic engine; immutable R1 v1.0.0 preserves the prior-session cutoff and R1 v1.1.0 includes the latest completed evaluation session. One-symbol latest-session manual local/fetch-and-freeze orchestration, `FACTOR_PREVIEW` Runs, exact Schema-v14 reload, background Factor-page controls, inspection/export and Open Run are verified. Both definitions are locked with `execution_allowed=false` and `live_allowed=false`. Full P23-1E comparison/scoring, wavelets and every cycle/state/target/Decision/Risk/Backtesting/Accounting/Execution consumer remain unimplemented.
...
- `quant_trading.persistence` — shared SQLite Schema v14/migration boundary plus independent Run History, generic/specialized Factor, Decision, Risk, Capital Allocation, Asset State, Target Position and P23-1 evidence/query adapters; see `docs/modules/central-persistence.md`.
...
| P23-1 Manual Spectral Preview | `IMPLEMENTED_VERIFIED_DISABLED` | exact R1 v1.1.0 latest-session windows, explicit local/read-only evidence preparation, one top-level Run, Schema-v14 reload and existing Factor-page GUI; no financial consumer |
...
- PROPOSAL-025 P23-1E-A is implemented and verified disabled. User-approved amendment B preserves immutable R1 v1.0.0 and adds immutable R1 v1.1.0, whose exact 60/120/250 windows include the latest completed evaluation session. No other spectral equation changed.
- The explicit manual coordinator prepares one symbol's exact latest completed XNYS evidence through `LOCAL_ONLY` or explicit-click `FETCH_AND_FREEZE_READ_ONLY`, then delegates to the existing Factor service. Preparation failures create searchable failed Runs; successful clicks create one top-level Run. The existing Factor-page panel uses a background worker, suppresses duplicate requests, reloads exact stored evidence and opens Run History.
- Schema remains v14/94 with `integrity_check=ok` and zero foreign-key violations. The one approved AAPL validation produced Run `97448eba-e403-4be9-96a9-5c6cf8b52695`, operation `5380fd0e-51c4-418f-ae8e-50a7ab42ba8e`, as-of `2026-07-31T20:00:00Z`, 250 observations and valid 60/120/250 windows. It reloaded from a fresh process. Candidate periods were 20, 40 and approximately 83.33 sessions; method disagreement left no qualified cross-window consensus, so the evidence makes no trading claim.
- Full verification passed 547 tests with one pre-existing upstream WebSocket warning; compilation passed. `BUG-20260802-001/002/003` were fixed with regression coverage. The external validation used Historical Stock Data and Corporate Actions only—no Trading client, account, position, order or fill access. No commit or push was performed.
...
`PROPOSAL-024` P23-1A–D and `PROPOSAL-025` P23-1E-A are implemented and verified as disabled/unconsumed specialized research. No further development slice is currently approved. Full P23-1E historical comparison/scoring, wavelets, P23-2 cycle/state semantics, P23-3 target curve/position values, P23-4 Decision/Risk linkage, P23-5 simulation, actual/default cap/floor/plan selection, factual Accounting/broker cash, complete Risk approval, Paper execution, production activation and Live require separate scope and approval. Paper/Live Execution boundaries remain empty.

## Last verified date

2026-08-02 (full suite 547 passed with one existing upstream warning; compilation and Schema-v14 integrity/foreign-key checks passed. PROPOSAL-025 P23-1E-A is the current implemented disabled research checkpoint. The one approved read-only AAPL latest-session validation and fresh-process reload succeeded under exact R1 v1.1.0. No Trading API, account, position, order or fill access occurred; Live and automatic submission remain disabled. See EDIT-20260802-002.)

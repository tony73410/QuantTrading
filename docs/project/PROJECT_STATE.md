...
- Central SQLite Schema v15 reuses `runtime/data/market_history.sqlite3` for independent Market/Run/Factor/P23-1/P26/Decision/Risk/Capital/State/Target evidence. Verified additive v1→v15 migrations preserve earlier evidence. The v14→v15 migration adds five P26 tables, expands the required logical schema from 94 to 99 tables, preserves prior row counts, passes integrity/foreign-key checks and backfills no study. Immutable locked R1 v1.0.0 and v1.1.0 definitions and existing AAPL P25 evidence remain reloadable. No production consumer is active.
...
- `quant_trading.persistence` — shared SQLite Schema v15/migration boundary plus independent Run History, generic/specialized Factor, Decision, Risk, Capital Allocation, Asset State, Target Position and P23-1/P26 evidence/query adapters; see `docs/modules/central-persistence.md`.
...
- `PROPOSAL-026` 已批准、实现并验证为禁用研究能力。2–250历史评估日、最多两个R1版本、`RETROSPECTIVE_ADJUSTED`语义、父子Run、Schema v15五表、显式本地/只读获取接口和现有Factor页历史GUI均已完成；本次未进行另一次真实AAPL网络验证。
...
`PROPOSAL-024` P23-1A–D, `PROPOSAL-025` P23-1E-A and `PROPOSAL-026` P23-1E-B are implemented and verified as disabled/unconsumed specialized research. No further development slice is currently approved. Wavelets, P23-2 cycle/state semantics, P23-3 target curve/position values, P23-4 Decision/Risk linkage, P23-5 predictive/trading simulation, actual/default cap/floor/plan selection, factual Accounting/broker cash, complete Risk approval, Paper execution, production activation and Live require separate scope and approval. Paper/Live Execution boundaries remain empty.

## Last verified date

2026-08-03 (PROPOSAL-026 P23-1E-B implementation is the current disabled research checkpoint. Deterministic/local tests cover exact grids/cutoffs, one-fetch evidence, parent/child Runs, complete membership, migration/reload, GUI/chart/export and architecture. Central Schema v15/99 integrity/foreign-key checks pass with zero study backfill. No new Alpaca network, Trading API, account, position, order or fill access occurred; Live and automatic submission remain disabled. See EDIT-20260802-005.)

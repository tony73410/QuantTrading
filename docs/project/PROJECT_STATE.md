...
Governed stock historical data browser plus an independent Algorithm Control Center with Phase 1–2B research observability, Phase 3A Research Capital Allocation, Phase 4A manual Asset State history, Phase 5A bounded Target Position, Phase 5B manual standardized-price-state research, Phase 5C exact source-to-target linkage, Phase 5D exact Target Adjustment Decision, Phase 6A/6B/6C/6D ordered Risk research previews, Phase 6E read-only consolidated Risk inspection and disabled P23-1 research through P23-1F. Central Schema v16 preserves all prior evidence, exact P23-1/P26 provenance and immutable daily-volatility-profile attempts/results/source traces. PROPOSAL-027 is implemented, verified, locked, disabled and unconsumed; its AAPL result is a descriptive daily scale, not a reversal threshold. Portfolio Accounting remains an independent in-memory fact-replay scaffold. Paper/Live Execution remain declaration-only; no active production algorithm, complete numerical Risk approval, broker account connection, order, or execution behavior exists.
...
- Central SQLite Schema v16 reuses `runtime/data/market_history.sqlite3` for independent Market/Run/Factor/P23-1/P26/P23-1F/Decision/Risk/Capital/State/Target evidence. Verified additive v1→v16 migrations preserve earlier evidence. The v15→v16 migration backed up v15/99, adds five P23-1F tables, expands the required logical schema to 104 tables, preserves prior row counts, passes integrity/foreign-key checks and backfills no profile. The approved local AAPL validation adds one definition, one attempt/result, 20 daily source rows and three window summaries; exact restart reload and Run lineage are verified. No production consumer is active.
...
- `quant_trading.persistence` — shared SQLite Schema v16/migration boundary plus independent Run History, generic/specialized Factor, Decision, Risk, Capital Allocation, Asset State, Target Position and P23-1/P26/P23-1F evidence/query adapters; see `docs/modules/central-persistence.md`.
...
- Active central SQLite is Schema v16/104 with `integrity_check=ok` and zero foreign-key violations. It retains one P26 evidence set, 270 observations, one study, two definition selections, 40 study points and 41 total P23-1 operations, and adds one P23-1F definition/attempt/result, 20 exact daily inputs and three window summaries. No network, Trading client, account, position, order or fill access occurred during P27 validation.
...
- 当前Phase 1–6E及P23-1A–F能力构成已验证研究检查点；项目包版本仍为`0.1.0`，P27代码与治理状态由包含`CHECKPOINT-20260810-005`的main提交发布。Phase 6E、P26与P27只增加获批的只读精确历史观察和日常波动档案，不改变三条有序且不预留资金的数值候选证据，也不授予运行激活、完整Risk批准或交易权限。

## Next approved work

`PROPOSAL-024` P23-1A–D, `PROPOSAL-025` P23-1E-A, `PROPOSAL-026` P23-1E-B and `PROPOSAL-027` P23-1F are implemented and verified as disabled/unconsumed specialized research. No further implementation slice is currently approved. A reversal multiplier, wavelets, P23-2 cycle/state semantics, P23-3 target curve/position values, P23-4 Decision/Risk linkage, P23-5 predictive/trading simulation, actual/default cap/floor/plan selection, factual Accounting/broker cash, complete Risk approval, Paper execution, production activation and Live require separate scope and approval. Paper/Live Execution boundaries remain empty.

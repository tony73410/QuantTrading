...
Governed stock historical data browser plus an independent Algorithm Control Center with Phase 1–2B research observability, Phase 3A Research Capital Allocation, Phase 4A manual Asset State history, Phase 5A bounded Target Position, Phase 5B manual standardized-price-state research, Phase 5C exact source-to-target linkage, Phase 5D exact Target Adjustment Decision, Phase 6A/6B/6C/6D ordered Risk research previews, Phase 6E read-only consolidated Risk inspection, disabled P23-1 research through P23-1F, disabled P23-2A reversal observation and disabled P23-3A cycle-aware Target Position. Central Schema v18 preserves all prior evidence. PROPOSAL-028 remains unchanged and retains one separately approved AAPL `M=1.5` validation. Approved PROPOSAL-029 is implemented and verified in the current unpublished working tree: one exact P28 result/Run/daily step plus explicit hypothetical Decimal USD context feeds the approved `LOWER_PRICE_HIGHER_TARGET` linear/finite-exponential/saturated research formula. Approved PROPOSAL-030 completed three local AAPL P29 previews using one immutable disabled no-default test configuration; all three are `VALID_LINEAR`, reload/recalculation matched and no network or downstream consumer was used. No formal-state mutation or Decision/Risk/cash/Backtesting/Accounting/Execution consumer exists. Portfolio Accounting remains an independent in-memory fact-replay scaffold. Paper/Live Execution remain declaration-only; no active production algorithm, complete numerical Risk approval, broker account connection, order, or execution behavior exists.

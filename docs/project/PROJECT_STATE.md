...
Governed stock historical data browser, restricted and versioned Factor authoring, separately testable Factor/Decision/Risk contracts, an independent Algorithm Control Center, and empty sibling Paper/Live Execution namespaces; no active production Decision/Risk algorithm, account connection, order, or execution behavior.
...
- Scheme A Factor authoring：GUI可保存受限表达式为不可变Factor版本，并在Decision配置中选择精确Factor版本；定义默认禁用，不能执行任意Python，也不会自动产生TradeIntent或订单。
...
- Factor persistence is implemented but inactive in normal application flows because no production Factor calculator is registered and Market History-to-Factor availability semantics remain unapproved.
...
- Factor/Decision/Risk尚未接入Market History或GUI正式运行流程；Factor结果持久化仅可通过显式Store注入使用，Decision/Risk持久化未实现。控制中心只管理元数据和配置，不代表算法可供实际交易。
- 初始仓库没有已保存的用户Factor；保存后Factor页会显示禁用版本。没有正式Decision/Risk策略，因此对应页仍可能为空或仅显示锁定安全项，Pipeline Dry Run不可运行。
- Risk的Account/Portfolio/OpenOrders Provider只有抽象Protocol；没有账户连接。Emergency de-risking当前只会暂停新Intent，自动平仓Not implemented。

## Next approved work

只批准建立空白Paper/Live Execution边界，没有批准向其中添加内容。`PROPOSAL-004` 已保存为六阶段后续方向的 **Draft, not approved for implementation**；存储清理为 **Proposed, not approved**。Paper execution行为、策略、回测和 Live 均不得自行开始。当前发布检查点见 [`VERSION_HISTORY.md`](VERSION_HISTORY.md)。

## Last verified date

2026-07-14 (publication checkpoint: Scheme A restricted Factor authoring, Decision Factor-version selection, central SQLite Factor history, empty Paper/Live boundaries and Proposal-004 Draft verified; 216 tests passed with one upstream warning; Live and automatic submission remain disabled.)

...
Governed stock historical data browser plus an independent Algorithm Control Center with Phase 1–2B research observability, Phase 3A Research Capital Allocation, Phase 4A manual Asset State history, Phase 5A bounded Target Position, Phase 5B manual standardized-price-state research, Phase 5C exact source-to-target linkage, Phase 5D exact Target Adjustment Decision, Phase 6A/6B/6C/6D ordered Risk research previews, Phase 6E read-only consolidated Risk inspection, disabled P23-1 research through P23-1F, disabled P23-2A reversal observation and disabled P23-3A cycle-aware Target Position. Central Schema v18 preserves all prior evidence. Main feature commit `7ad1e1f` contains approved PROPOSAL-029 and completed PROPOSAL-030: one exact P28 result/Run/daily step plus explicit hypothetical Decimal USD context feeds the approved `LOWER_PRICE_HIGHER_TARGET` linear/finite-exponential/saturated formula, and three local AAPL previews using one immutable disabled no-default test configuration are all `VALID_LINEAR` with exact reload/recalculation. No network, formal-state mutation or Decision/Risk/cash/Backtesting/Accounting/Execution consumer was used. Portfolio Accounting remains an independent in-memory fact-replay scaffold. Paper/Live Execution remain declaration-only; no active production algorithm, complete numerical Risk approval, broker account connection, order, or execution behavior exists.
...
- Central SQLite Schema v18 reuses `runtime/data/market_history.sqlite3` for independent Market/Run/Factor/P23-1/P26/P23-1F/P23-2/P23-3A/Decision/Risk/Capital/State/Target evidence. Verified additive v1→v18 migrations preserve earlier evidence. The v17→v18 migration created backup `market_history.schema-v17-to-v18.20260811T031305700700Z.sqlite3`, expanded 110→116 with exactly six P29 tables, changed no prior business-table row count, passed backup/active integrity and foreign-key checks and backfilled no P29 record. P30 then intentionally appended one formula, one AAPL configuration, five attempts/Runs, three results/traces and eighteen source links after backup `market_history.before-p30-validation.20260811T0428081654404Z.sqlite3`; every unrelated table count remained unchanged and no production consumer is active.
...
- `quant_trading.persistence` — shared SQLite Schema v18/migration boundary plus independent Run History, generic/specialized Factor, Decision, Risk, Capital Allocation, Asset State, Target Position and P23-1/P26/P23-1F/P23-2/P23-3A evidence/query adapters; see `docs/modules/central-persistence.md`.
...
| P23-3A Cycle-Aware Target Position | `IMPLEMENTED_VERIFIED_DISABLED` | 精确P28 Result/Run/Step、反向线性/有限指数/饱和公式、无默认不可变配置、Schema v18重载/重放/导出与既有Target Position页子页签；六张P29表为空且无下游消费者 |
...
- `PROPOSAL-029` 已获用户对推荐P29-D1–D10完整包的批准并实现验证为禁用且无消费者的P23-3A能力：既有Target Position所有者读取一个显式精确P28 Result/Run/Step，使用`LOWER_PRICE_HIGHER_TARGET`方向、`x=ln(P/R)/k`、线性/导数匹配有限指数/饱和互斥区域和精确Decimal/IEEE证据。Schema v18、Run/结果历史、重放/比较/导出和既有Target Position页兄弟子页签均已完成；没有真实公式/股票参数/默认值/结果，也没有P23-4消费者。
...
- Active central SQLite is Schema v18/116 with `integrity_check=ok` and zero foreign-key violations. Verified migration backup `market_history.schema-v17-to-v18.20260811T031305700700Z.sqlite3` remains v17/110. P30 preflight backup `market_history.before-p30-validation.20260811T0428081654404Z.sqlite3` remains the zero-row P29 baseline. Active P29 counts are formula `1`, configuration `1`, attempts `5`, results `3`, traces `3`, source links `18`; exactly five Runs/eight stages/four symbol-index rows/twelve bindings were appended and every unrelated table count was preserved. Existing P26/P27/P28 evidence remains immutable; no network, Trading client, account, position, order or fill access occurred.
...
None. `PROPOSAL-024` through `PROPOSAL-029` are implemented and verified as disabled/unconsumed specialized research, and approved `PROPOSAL-030` has completed its bounded local `DRY_RUN`. P23-2A retains its explicit non-default AAPL `M=1.5` validation; P23-3A now has one explicit disabled AAPL test configuration (`0.20/0.50/0.80`, `s=0.05`, `A=2`, `B=4`) and three independent hypothetical linear results over exact existing P28 evidence. These are not defaults, sequential holdings or an approved consumer. Wavelets, formal P23-2 state mutation, P23-4 Decision/Risk linkage, P23-5 predictive/trading simulation, actual/default cap/floor/plan selection, factual Accounting/broker cash, complete Risk approval, Paper execution, production activation and Live all require separate scope and approval. Paper/Live Execution boundaries remain empty.

...
Governed stock historical data browser plus an independent Algorithm Control Center with research observability through disabled P23-4A. Approved PROPOSAL-031 now converts one explicitly selected accepted P29 Result/Run into a type-distinct `INCREASE`/`DECREASE`/`HOLD` Decision preview using the same exact-difference kernel as Phase 5D, while preserving old Phase 5D history. Read-only preflight, immutable history, deterministic replay/export, Run navigation and the existing Decision-page inspector are implemented. Central Schema v19 preserves all prior evidence and adds four empty P31 tables; no P30 result was automatically converted. No Risk/cash/Backtesting/Accounting/Paper/Live/order consumer or execution authority was added. Portfolio Accounting remains an independent in-memory fact-replay scaffold; no active production algorithm, complete numerical Risk approval, broker account connection, order or execution behavior exists.
...
- Central SQLite Schema v19 reuses `runtime/data/market_history.sqlite3` for independent Market/Run/Factor/P23-1/P26/P23-1F/P23-2/P23-3A/P23-4A/Decision/Risk/Capital/State/Target evidence. Verified additive v1→v19 migrations preserve earlier evidence. The v18→v19 migration created backup `market_history.schema-v18-to-v19.20260811T191208556475Z.sqlite3`, expanded 116→120 with exactly four P31 tables, preserved `algorithm_runs=54` and P29 formula/configuration/attempt/result counts `1/1/5/3`, passed backup/active integrity and foreign-key checks and backfilled no P31 record.
...
- P23-4A Cycle-Target Decision Preview is `IMPLEMENTED_VERIFIED_DISABLED`: component `decision.cycle_target_adjustment.p23_4a.v1@1.0.0` accepts one explicit accepted P29 Result/Run, validates exact P29/P28 lineage and arithmetic, and emits one type-distinct non-executable intent for a nonzero difference or HOLD with none for exact zero. It has no default source or Risk/downstream consumer; all four Schema-v19 P31 tables remain empty.
...
- `quant_trading.persistence` — shared SQLite Schema v19/migration boundary plus independent Run History, generic/specialized Factor, Decision, Risk, Capital Allocation, Asset State, Target Position and P23-1/P26/P23-1F/P23-2/P23-3A/P23-4A evidence/query adapters; see `docs/modules/central-persistence.md`.
...
| P23-4A Cycle-Target Decision Preview | `IMPLEMENTED_VERIFIED_DISABLED` | 显式P29 Result/Run、共享精确差额内核、类型独立的零或一条intent、Schema v19重载/重放/导出与既有Decision页子页签；四张P31表为空，止于Risk之前 |
...
- `PROPOSAL-029` 已获用户对推荐P29-D1–D10完整包的批准并实现验证为禁用P23-3A能力：既有Target Position所有者读取一个显式精确P28 Result/Run/Step，使用`LOWER_PRICE_HIGHER_TARGET`方向、`x=ln(P/R)/k`、线性/导数匹配有限指数/饱和互斥区域和精确Decimal/IEEE证据。Schema v18、Run/结果历史、重放/比较/导出和既有Target Position页兄弟子页签均已完成；P30保存一项禁用AAPL测试配置和三条线性验证结果，P31是唯一获批的显式禁用Decision消费者，仍没有默认选择或Risk消费者。
- `PROPOSAL-031` 已获用户对推荐P31-D1–D10完整包的批准并实现验证为禁用P23-4A能力：旧Phase 5D合同/历史保持不变，两条Decision路径共享一个纯精确差额内核；应用层只解析显式P29 Result/Run，保存类型独立证据并提供只读preflight/replay/export/Run导航和Decision页检查器。Schema v19四表零回填且当前仍为空；Risk和所有交易下游未批准。
...
- Active central SQLite is Schema v19/120 with `integrity_check=ok` and zero foreign-key violations. Verified migration backup `market_history.schema-v18-to-v19.20260811T191208556475Z.sqlite3` remains v18/116 and has the same integrity result. Active `algorithm_runs=54`; P29 formula/configuration/attempt/result counts remain `1/1/5/3`; each of the four P31 tables has zero rows. Existing P26/P27/P28/P29 evidence remains immutable; no network, Trading client, account, position, order or fill access occurred.
...
None. `PROPOSAL-024` through `PROPOSAL-031` are implemented and verified as disabled specialized research, and approved `PROPOSAL-030` has completed its bounded local `DRY_RUN`. P23-4A has code, Schema-v19 persistence and GUI/replay support but deliberately has zero runtime P31 result and no Risk consumer. A first explicit P31 local validation, P23-4 Risk/count/freeze linkage, wavelets, formal P23-2 state mutation, P23-5 predictive/trading simulation, actual/default cap/floor/plan selection, factual Accounting/broker cash, complete Risk approval, Paper execution, production activation and Live all require separate scope and approval. Paper/Live Execution boundaries remain empty.

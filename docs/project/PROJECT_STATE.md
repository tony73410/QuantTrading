...
Governed stock historical data browser plus an independent Algorithm Control Center with Phase 1–2B research observability, Phase 3A Research Capital Allocation, Phase 4A manual Asset State history, Phase 5A bounded Target Position, Phase 5B manual standardized-price-state research, Phase 5C exact source-to-target linkage, Phase 5D exact Target Adjustment Decision, Phase 6A/6B/6C/6D ordered Risk research previews, Phase 6E read-only consolidated Risk inspection, disabled P23-1 research through P23-1F and disabled P23-2A reversal observation. Central Schema v17 preserves all prior evidence; its six P23-2 tables began with zero backfill and now retain one separately approved AAPL validation. PROPOSAL-028 is implemented and verified: one explicit user-created positive multiplier is shared by both directions over exact forward-frozen P27/local evidence, with inclusive log thresholds, two-session confirmation and next-session activation. The validation used explicit `M=1.5` but created no default, formal-state mutation or financial/trading consumer. Portfolio Accounting remains an independent in-memory fact-replay scaffold. Paper/Live Execution remain declaration-only; no active production algorithm, complete numerical Risk approval, broker account connection, order, or execution behavior exists.
...
- Central SQLite Schema v17 reuses `runtime/data/market_history.sqlite3` for independent Market/Run/Factor/P23-1/P26/P23-1F/P23-2/Decision/Risk/Capital/State/Target evidence. Verified additive v1→v17 migrations preserve earlier evidence. The v16→v17 migration created backup `market_history.schema-v16-to-v17.20260810T192850337602Z.sqlite3`, expanded 104→110 with exactly six P23-2 tables, changed no prior business-table row count, passed backup/active integrity and foreign-key checks and backfilled no P28 record. The later approved validation appended 1 disabled definition, 2 attempts, 1 result, 3 daily steps, 0 events and 9 source links; no production consumer is active.
...
- `quant_trading.persistence` — shared SQLite Schema v17/migration boundary plus independent Run History, generic/specialized Factor, Decision, Risk, Capital Allocation, Asset State, Target Position and P23-1/P26/P23-1F/P23-2 evidence/query adapters; see `docs/modules/central-persistence.md`.
...
- Active central SQLite is Schema v17/110 with `integrity_check=ok` and zero foreign-key violations. Verified backup `market_history.schema-v16-to-v17.20260810T192850337602Z.sqlite3` remains v16/104 and also passes integrity/foreign-key checks. All 104 prior logical-table row counts were preserved by migration. The later approved P28 validation now accounts for 1 definition, 2 attempts, 1 result, 3 daily steps, 0 events and 9 source links. Existing P26/P27 evidence remains immutable; the validation used Alpaca Market Data only and no Trading client, account, position, order or fill access occurred.
...
`PROPOSAL-024` through `PROPOSAL-028` are implemented and verified as disabled/unconsumed specialized research. P23-2A now supplies exact symmetric reversal observations, persistence and inspection, and one separately approved AAPL validation completed with explicit `M=1.5`; that multiplier is not a default. The user has authorized creation of proposal-only `PROPOSAL-029` for P23-3 bounded linear/accelerated target-position mathematics after the P28 publication, but no P29 runtime implementation or parameter/default is approved. Wavelets, formal P23-2 state mutation, P23-4 Decision/Risk linkage, P23-5 predictive/trading simulation, actual/default cap/floor/plan selection, factual Accounting/broker cash, complete Risk approval, Paper execution, production activation and Live require separate scope and approval. Paper/Live Execution boundaries remain empty.

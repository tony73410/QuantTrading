...
Current status (2026-08-13): `PROPOSAL-024` through `PROPOSAL-031`, `PROPOSAL-033` and `PROPOSAL-035` are approved and implemented as disabled research slices. Main commit `b147e60` publishes approved P35-D1–D10 as P23-4C1 on Schema v21/130 together with all earlier P31/P32/P33/P34 evidence. `PROPOSAL-036` is approved and completed as a bounded `DRY_RUN`: one explicit first AAPL `ELIGIBLE` event and three exact P35 manual-review-only results now provide restart-safe local evidence. P23-4C2 daily-opportunity counting remains pending and unapproved until truthful logical action/fill facts exist.

...
Current status (2026-08-12): `PROPOSAL-024` through `PROPOSAL-031`, `PROPOSAL-033` and `PROPOSAL-035` are approved and implemented as disabled research slices. P31/P32/P33/P34 are published through CHECKPOINT-20260812-007 at commit `828f956`; the current working tree implements approved P35-D1–D10 as P23-4C1 only on Schema v21/130. No P35 runtime validation row exists, and P23-4C2 daily-opportunity counting remains pending and unapproved until truthful logical action/fill facts exist.
...
- [`PROPOSAL-035`](PROPOSAL-035-versioned-frozen-asset-admission-and-daily-opportunity-semantics.md) — approved option A / P35-D1–D10 and `IMPLEMENTED_VERIFIED_DISABLED` for P23-4C1. Asset State owns append-only `ELIGIBLE`/`FROZEN` evidence; exact P33 plus the exact effective neutral control enters a type-distinct block/manual-review-only Risk gate. Central Schema v21 adds six empty tables with zero backfill, and existing Asset State/Risk pages provide history, comparison, export and Run navigation. No runtime P35 result, approved output, daily counter or trading authority exists; P23-4C2 remains pending.

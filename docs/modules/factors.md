...
P23-1 is deliberately specialized rather than a generic scalar `FactorSnapshot`. Immutable R1 v1.0.0 retains component ID `factor.spectral_volatility.p23_1_r1.v1`, semantic version `1.0.0`, definition ID `7d6974fe-d579-5cc3-bf91-0940976992b3` and its prior-session window cutoff. PROPOSAL-025 adds immutable R1 v1.1.0 under a distinct definition identity; it includes the latest completed evaluation session in every exact 60/120/250-session window. No OLS, MAD, Welch, full-window, ambiguity, amplitude, residual or cross-window equation changed. The service records `MARKET_DATA` then `FACTOR` stages under `FACTOR_PREVIEW` / `NO_EXECUTION`; P26 adds only an optional parent Run identity so the same service still creates every child Run and operation. It has no State, Target Position, Decision, Risk, Backtesting, Accounting or Execution consumer. IEEE-754 hexadecimal values preserve exact replay in addition to readable numeric values.

`SpectralHistoricalDefinitionSelection`, `SpectralHistoricalStudyPoint`, `SpectralHistoricalStudy` and bounded query contracts preserve the P26 date×definition grid without duplicating numerical truth. They contain statuses, exact IDs/versions, child Run/operation references, warnings/errors and full-denominator counts; they contain no future return, P&L, score, rank, selected winner or trading field. Detailed period/amplitude/MAD evidence is always reloaded from the referenced immutable `SpectralVolatilityOperation`.

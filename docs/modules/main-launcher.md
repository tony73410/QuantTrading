...
The trusted application catalog exposes Market History, Algorithm Control, and Backtesting & Simulation. A separate static shortcut catalog covers Idea Notebook, Asset Factor, Market Factor, Decision, Risk, Execution status, Portfolio & Ledger, Simulation Strategies, Pipeline, Conflict Center and Audit. Every shortcut opens the existing Algorithm Control process with a reviewed `--page` value; it does not reimplement the page.
...
`tests/unit/launcher/test_main_launcher.py` verifies the three child applications, all eleven core-page shortcuts, trusted-module/static-argument restrictions, detached command construction, direct-page selection and visible safety state.

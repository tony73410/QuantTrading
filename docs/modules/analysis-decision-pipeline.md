...
Coordinate explicitly approved cross-owner research call order while keeping every domain engine independently usable. Current paths include Factor then Decision and optional Risk, exact Standardized State into finite-knot Target Position, one exact completed linked target into the type-distinct Decision preview, one exact P28 Result/Run/Daily Step into P23-3A Target Position, and one exact P37 terminal state through exact P28 validation into unchanged P29.
...
- Resolve one exact successful P37 operation/Run/stream/terminal snapshot, verify its definition/cycle/source link and exact backing P28 identities/semantics, then call the existing P29 runner with a distinct fixed target operation ID. Persist only the P39 bridge attempt/link; do not calculate state/target math or call P31.

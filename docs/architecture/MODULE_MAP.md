...
| `quant_trading.persistence` | Shared local SQLite Schema v17, versioned backup/migration, research queries, Run History repository and immutable generic/P23-1/P26/P23-1F/P23-2/Decision/Risk/Capital/Asset State/Target Position evidence adapters | `CentralSQLiteDatabase`, existing repositories/query services, `SQLiteSpectralVolatilityStore`, `SQLiteSpectralHistoricalStudyStore`, `SQLiteDailyVolatilityProfileStore`, `SQLiteReversalObservationStore` and specialized research stores | stdlib sqlite3, neutral Run History and public domain contracts | Market Store initialization; Orchestration/domain composition; typed research inspectors | `docs/modules/central-persistence.md` |
...
| `quant_trading.algorithm_control` | Registry/ParameterSchema management, immutable versions, safe previews/audit and typed research owner pages, including P26/P23-1F inspection and the P23-2 laboratory inside the existing Asset State page | `AlgorithmControlController`, `AlgorithmControlPanel`, typed panels, chart/export services, `python -m quant_trading.algorithm_control` | stdlib, PySide6, Plotly, shared renderer and approved public service/query contracts | Desktop user; local preview and explicit research management | `docs/modules/algorithm-control-gui.md` |

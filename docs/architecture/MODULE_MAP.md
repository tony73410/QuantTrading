...
| `quant_trading.persistence` | Shared local SQLite Schema v22, versioned backup/migration, Run History and immutable generic/P23-1 through P23-4C1 plus P23-2B evidence adapters | `CentralSQLiteDatabase`, existing repositories/query services and specialized research stores including `SQLiteMathematicalCycleStateStore` | stdlib sqlite3, neutral Run History and public domain contracts | composition roots and typed research inspectors | `docs/modules/central-persistence.md` |
...
| `quant_trading.factors` | Single-asset Factor contracts plus disabled standardized-state, P23-1 spectral and P23-1F daily-profile research; owns typed exact-source evidence and robust daily-scale meaning; no production formula | Factor contracts/engines plus `SpectralVolatilityService`, `DailyVolatilityProfileService` and typed Store/query contracts | stdlib/NumPy, public MarketBar/calendar evidence and neutral Run History contracts | Orchestration, Persistence adapters, Algorithm Control or independent research caller | `docs/modules/factors.md`, `docs/modules/standardized-price-state.md` |
| `quant_trading.decision` | Public FactorSnapshot to generic non-executing DecisionResult/TradeIntent plus isolated exact Phase 5D and P23-4A Target Adjustment Decisions sharing one exact-difference kernel; owns immutable trace/history meaning; no production policy | generic/Phase-5D contracts plus `CycleTargetAdjustmentDecisionService`, type-distinct P31 result/intent and Store/query contracts | stdlib, Factor public contracts for generic decisions; neutral Run/source DTO contracts for specialized paths | Orchestration, Persistence adapters, Algorithm Control or independent decision caller | `docs/modules/trading-decision.md` |
| `quant_trading.risk` | Generic conservative Risk contracts/engine, isolated Phase 6A/P23-4B structural gates, disabled P23-4C1 frozen-asset admission and Phase 6B/6C/6D ordered numerical previews; no complete/production numerical approval policy | generic Risk contracts plus specialized target-adjustment, cycle-target admission, exposure-cap, cash-floor and research-asset-cash services with source-neutral rule/result/Store/query contracts | stdlib, safe application role enums and neutral Run identity | Orchestration, Persistence adapter, Algorithm Control inspector | `docs/modules/risk-control.md` |
| `quant_trading.orchestration` | Organizes approved cross-owner call order, including exact Standardized State → Target, exact P28 step → P23-3A Target, explicit P29 Result/Run → P23-4A Decision, exact P31 intent/result/Run → P23-4B structural Risk, exact P33/control evidence → P23-4C1 admission, linked Target → Phase 5D Decision and its intent → structural Risk review; contains no formulas or rule outcomes | existing pipelines/coordinators plus cycle-target position/Decision/Risk/admission and asset-trading-control coordinators | public Factor/P28/P29/P31/P33/Decision/Risk/Asset State/Target Position/Run contracts | Algorithm Control and future approved application services | `docs/modules/analysis-decision-pipeline.md` |
...
| `quant_trading.asset_state` | User-defined symbolic state graphs/manual history, isolated disabled P23-2 reversal observation and separate P23-4C1 append-only `ELIGIBLE/FROZEN` trading-control authority | `AssetStateService`, `ReversalObservationService`, `AssetTradingControlService` and typed public query/contracts | stdlib, shared errors, neutral Run History contracts | SQLite adapter, application evidence orchestration, Algorithm Control owner page and exact neutral P35 admission bridge only; no execution consumer | `docs/modules/asset-state.md` |
| `quant_trading.target_position` | Existing finite-knot/manual/linked research plus separately versioned exact-P28-step P23-3A bounded linear/finite-exponential targets, traces and replay | existing Target/linked services plus `CycleTargetPositionService`, engine/replay, Store/query and typed schema-v1 contracts | stdlib, shared errors, neutral Run History contracts | SQLite adapter, Phase 5C/P23-3 orchestration, Algorithm Control and the exact P23-4A public-source adapter only | `docs/modules/target-position.md` |
| `quant_trading.algorithm_control` | Registry/ParameterSchema management, immutable versions, safe previews/audit and typed research owner pages, including P26/P23-1F/P23-2/P23-3A plus sibling P23-4A Decision, P23-4B Risk and P23-4C1 control/admission inspectors | `AlgorithmControlController`, `AlgorithmControlPanel`, typed panels, chart/export services, `python -m quant_trading.algorithm_control` | stdlib, PySide6, Plotly, shared renderer and approved public service/query contracts | Desktop user; local preview and explicit research management | `docs/modules/algorithm-control-gui.md` |
...
| P23-1 Spectral Historical Research | `IMPLEMENTED_VERIFIED_DISABLED` | exact R1 v1.0.0/v1.1.0, P25 explicit latest-session runner, P26 one-symbol 2–250-session parent/child history, Schema v15 evidence and existing Factor-page inspection; no prediction or financial consumer |
| P23-1F Daily Volatility Profile | `IMPLEMENTED_VERIFIED_DISABLED` | explicit whole P26/R1 v1.0.0 source, robust daily/window medians and temporal MAD, secondary-only spectral summaries, Schema v16/Run lineage/export and existing Factor-page subtab; no threshold, state, target, Decision, Risk or execution consumer |
| P23-2A Symmetric Reversal Observation | `IMPLEMENTED_VERIFIED_DISABLED` | one explicit same multiplier, exact positive P27 and forward-frozen local evidence, inclusive log threshold, two-session confirmation/next-session activation, Schema v17 replay/export and existing Asset State subtab; no formal-state or financial consumer |
| P23-3A Cycle-Aware Bounded Target Position | `IMPLEMENTED_VERIFIED_DISABLED` | exact P28 Result/Run/Step, `ln(P/R)/k`, versioned contrarian linear/finite-exponential/saturated response, exact Decimal/IEEE trace, Schema v18 replay/export and existing Target Position sibling subtab; P30 created one disabled AAPL configuration and three results; only explicit P23-4A consumption is approved |
| P23-4A Cycle-Target Decision Preview | `IMPLEMENTED_VERIFIED_DISABLED` | explicit accepted P29 Result/Run, read-only preflight, shared exact positive/negative/zero Decision kernel, zero-or-one type-distinct intent, Schema v19/Run/export and existing Decision sibling subtab; P32 added three exact independent AAPL validation results and P33 is their sole structural Risk consumer |
| P23-4B Cycle-Target Structural Risk Review | `IMPLEMENTED_VERIFIED_DISABLED` | explicit P31 Intent/Result/Run, shared private structural Risk kernel with old Phase 6A, type-distinct P31/P29/P28 provenance, manual-review/block only, Schema v20/Run/replay/export and existing Risk sibling subtab; P34 added three exact local manual-review results, with no numerical/downstream consumer |
| P23-4C1 Frozen-Asset Admission | `IMPLEMENTED_VERIFIED_DISABLED` | append-only Asset State control events, immediate freeze/next-session unfreeze/no default, exact P33 plus effective-control Risk admission, Schema v21/Run/replay/export and existing Asset State/Risk sibling subtabs; no runtime row, daily count, approved output or downstream consumer |
| P23-2B Mathematical Cycle State | `IMPLEMENTED_VERIFIED_DISABLED` | explicit exact cumulative-P28 promotion, day1/day2 old direction, day3 activation, prior-extreme reference, immutable snapshots/attribution-resolution events, Schema v22/Run/replay and read-only existing-page inspector; zero runtime stream and no consumer |
...
Cycle-Target Decision is a compatible Decision-owned sibling flow: `explicit accepted P29 Result + exact Run → read-only preflight → shared exact signed-difference kernel → P31 INCREASE/DECREASE/HOLD → type-distinct zero-or-one intent → typed GUI/Run History`. It created no initial runtime row; separately approved P32 later added three immutable validation results. It still cannot enter Phase 6A, generic Risk, cash, Backtesting, Accounting or Execution.

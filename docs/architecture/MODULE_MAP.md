...
| `quant_trading.persistence` | Shared local SQLite Schema v8, versioned backup/migration, research queries, Run History repository and immutable Decision/Risk/Capital/Asset State/Target Position/standardized-state/link evidence adapters | `CentralSQLiteDatabase`, existing repositories/query services, `SQLiteTargetPositionStore`, `SQLiteStandardizedPriceStateStore` | stdlib sqlite3, neutral Run History and public domain contracts | Market Store initialization; Orchestration/Capital/State/Target/standardized-state composition; typed research inspectors | `docs/modules/central-persistence.md` |
...
| `quant_trading.orchestration` | 组织已批准的跨所有者调用顺序：Factor → Decision → optional Risk，以及 exact Standardized State → Target Position；不含公式 | `AnalysisDecisionPipeline`, `TradingEvaluationPipeline`, `StandardizedStateTargetPositionPreviewCoordinator` | public Factor/Decision/Risk/Target Position/Run contracts | Algorithm Control and future approved application services | `docs/modules/analysis-decision-pipeline.md` |
...
| `quant_trading.algorithm_control` | Registry/ParameterSchema management, immutable Factor/Decision versions, safe previews/audit, typed inspectors and Capital/Asset State/Target Position/Standardized State owner pages | `AlgorithmControlController`, `AlgorithmControlPanel`, typed panels, chart/export services, `python -m quant_trading.algorithm_control` | stdlib, PySide6, Plotly, shared renderer and approved public service/query contracts | Desktop user; local preview and explicit research management | `docs/modules/algorithm-control-gui.md` |

...
| `quant_trading.persistence` | Shared local SQLite Schema v18, versioned backup/migration, research queries, Run History repository and immutable generic/P23-1/P26/P23-1F/P23-2/P23-3A/Decision/Risk/Capital/Asset State/Target Position evidence adapters | `CentralSQLiteDatabase`, existing repositories/query services, `SQLiteSpectralVolatilityStore`, `SQLiteSpectralHistoricalStudyStore`, `SQLiteDailyVolatilityProfileStore`, `SQLiteReversalObservationStore`, `SQLiteCycleTargetPositionStore` and specialized research stores | stdlib sqlite3, neutral Run History and public domain contracts | Market Store initialization; Orchestration/domain composition; typed research inspectors | `docs/modules/central-persistence.md` |
...
| `quant_trading.orchestration` | Organizes approved cross-owner call order, including exact Standardized State → Target, exact P28 step → P23-3A Target, linked Target → specialized Decision and specialized Decision intent → structural Risk review; contains no formulas or rule outcomes | existing pipelines/coordinators plus `CycleTargetPositionResearchCoordinator` and `TargetAdjustmentRiskReviewCoordinator` | public Factor/P28/Decision/Risk/Target Position/Run contracts | Algorithm Control and future approved application services | `docs/modules/analysis-decision-pipeline.md` |
...
| `quant_trading.target_position` | Existing finite-knot/manual/linked research plus separately versioned exact-P28-step P23-3A bounded linear/finite-exponential targets, traces and replay | existing Target/linked services plus `CycleTargetPositionService`, engine/replay, Store/query and typed schema-v1 contracts | stdlib, shared errors, neutral Run History contracts | SQLite adapter, Phase 5C/P23-3 orchestration and Algorithm Control owner page only; no P23-4/trading consumer | `docs/modules/target-position.md` |
| `quant_trading.algorithm_control` | Registry/ParameterSchema management, immutable versions, safe previews/audit and typed research owner pages, including P26/P23-1F/P23-2 and the P23-3A sibling Target Position inspector | `AlgorithmControlController`, `AlgorithmControlPanel`, typed panels, chart/export services, `python -m quant_trading.algorithm_control` | stdlib, PySide6, Plotly, shared renderer and approved public service/query contracts | Desktop user; local preview and explicit research management | `docs/modules/algorithm-control-gui.md` |

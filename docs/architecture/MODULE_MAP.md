...
| `quant_trading.persistence` | Shared local SQLite Schema v19, versioned backup/migration, research queries, Run History repository and immutable generic/P23-1/P26/P23-1F/P23-2/P23-3A/P23-4A/Decision/Risk/Capital/Asset State/Target Position evidence adapters | `CentralSQLiteDatabase`, existing repositories/query services, `SQLiteSpectralVolatilityStore`, `SQLiteSpectralHistoricalStudyStore`, `SQLiteDailyVolatilityProfileStore`, `SQLiteReversalObservationStore`, `SQLiteCycleTargetPositionStore`, `SQLiteCycleTargetAdjustmentDecisionStore` and specialized research stores | stdlib sqlite3, neutral Run History and public domain contracts | Market Store initialization; Orchestration/domain composition; typed research inspectors | `docs/modules/central-persistence.md` |
...
| `quant_trading.orchestration` | Organizes approved cross-owner call order, including exact Standardized State → Target, exact P28 step → P23-3A Target, explicit P29 Result/Run → P23-4A Decision, linked Target → Phase 5D Decision and its intent → structural Risk review; contains no formulas or rule outcomes | existing pipelines/coordinators plus `CycleTargetPositionResearchCoordinator`, `CycleTargetAdjustmentDecisionPreviewCoordinator` and `TargetAdjustmentRiskReviewCoordinator` | public Factor/P28/P29/Decision/Risk/Target Position/Run contracts | Algorithm Control and future approved application services | `docs/modules/analysis-decision-pipeline.md` |
...
| `quant_trading.algorithm_control` | Registry/ParameterSchema management, immutable versions, safe previews/audit and typed research owner pages, including P26/P23-1F/P23-2/P23-3A plus the sibling P23-4A Decision inspector | `AlgorithmControlController`, `AlgorithmControlPanel`, typed panels, chart/export services, `python -m quant_trading.algorithm_control` | stdlib, PySide6, Plotly, shared renderer and approved public service/query contracts | Desktop user; local preview and explicit research management | `docs/modules/algorithm-control-gui.md` |
...
Cycle-Target Decision is a compatible Decision-owned sibling flow: `explicit accepted P29 Result + exact Run → read-only preflight → shared exact signed-difference kernel → P31 INCREASE/DECREASE/HOLD → type-distinct zero-or-one intent → typed GUI/Run History`. It creates no initial runtime row and cannot enter Phase 6A, generic Risk, cash, Backtesting, Accounting or Execution.

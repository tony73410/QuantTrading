...
| `quant_trading.persistence` | Shared local SQLite Schema v20, versioned backup/migration, research queries, Run History repository and immutable generic/P23-1/P26/P23-1F/P23-2/P23-3A/P23-4A/P23-4B/Decision/Risk/Capital/Asset State/Target Position evidence adapters | `CentralSQLiteDatabase`, existing repositories/query services, `SQLiteSpectralVolatilityStore`, `SQLiteSpectralHistoricalStudyStore`, `SQLiteDailyVolatilityProfileStore`, `SQLiteReversalObservationStore`, `SQLiteCycleTargetPositionStore`, `SQLiteCycleTargetAdjustmentDecisionStore`, `SQLiteCycleTargetRiskStore` and specialized research stores | stdlib sqlite3, neutral Run History and public domain contracts | Market Store initialization; Orchestration/domain composition; typed research inspectors | `docs/modules/central-persistence.md` |
...
| `quant_trading.orchestration` | Organizes approved cross-owner call order, including exact Standardized State → Target, exact P28 step → P23-3A Target, explicit P29 Result/Run → P23-4A Decision, exact P31 intent/result/Run → P23-4B structural Risk, linked Target → Phase 5D Decision and its intent → structural Risk review; contains no formulas or rule outcomes | existing pipelines/coordinators plus `CycleTargetPositionResearchCoordinator`, `CycleTargetAdjustmentDecisionPreviewCoordinator`, `CycleTargetRiskReviewCoordinator` and `TargetAdjustmentRiskReviewCoordinator` | public Factor/P28/P29/P31/Decision/Risk/Target Position/Run contracts | Algorithm Control and future approved application services | `docs/modules/analysis-decision-pipeline.md` |
...
| `quant_trading.asset_state` | User-defined symbolic state graphs/manual history plus an isolated disabled P23-2 symmetric reversal-observation laboratory | `AssetStateService` and manual contracts; `ReversalObservationService`, pure evaluator, typed exact-evidence/result/replay contracts | stdlib, shared errors, neutral Run History contracts | SQLite adapter, application evidence orchestration and Algorithm Control owner page only; no financial or execution consumer | `docs/modules/asset-state.md` |
...
| `quant_trading.algorithm_control` | Registry/ParameterSchema management, immutable versions, safe previews/audit and typed research owner pages, including P26/P23-1F/P23-2/P23-3A plus sibling P23-4A Decision and P23-4B Risk inspectors | `AlgorithmControlController`, `AlgorithmControlPanel`, typed panels, chart/export services, `python -m quant_trading.algorithm_control` | stdlib, PySide6, Plotly, shared renderer and approved public service/query contracts | Desktop user; local preview and explicit research management | `docs/modules/algorithm-control-gui.md` |

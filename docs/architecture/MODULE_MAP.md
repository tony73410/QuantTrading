...
| `quant_trading.run_history` | Neutral NO EXECUTION research-run identity, lifecycle, ordered stages, exact bindings, messages and typed query views | `AlgorithmRunService`, `RunHistoryRepository`, `RunHistoryQueryService`, run/stage/detail models | stdlib only | Orchestration, Persistence adapters, Algorithm Control read-only GUI | `docs/modules/run-history.md` |
| `quant_trading.persistence` | Shared local SQLite Schema v6, versioned backup/migration, Factor/Decision research queries, Run History repository and immutable Decision/Risk/Capital/Asset State/Target Position evidence adapters | `CentralSQLiteDatabase`, `SQLiteFactorSnapshotStore`, `SQLiteRunHistoryRepository`, `SQLiteAlgorithmResultStore`, `SQLiteResearchHistoryQueryService`, `SQLiteCapitalAllocationStore`, `SQLiteAssetStateStore`, `SQLiteTargetPositionStore` | stdlib sqlite3, neutral Run History and public domain contracts | Market Store initialization; Orchestration/Capital/State/Target composition; typed research inspectors | `docs/modules/central-persistence.md` |
...
| `quant_trading.factors` | Single-asset, as-of-safe Market Data to versioned strategy-neutral FactorSnapshot; owns typed history/filter/exact-version and exact source-price evidence meaning; no production formula | `FactorCalculator`, `SingleAssetFactorEngine`, `FactorSnapshot`, `FactorHistoryQueryService`, `FactorVisualizationQueryService` and typed records | stdlib, public MarketBar/dimension models | Orchestration, Persistence adapters or independent research caller | `docs/modules/factors.md` |
| `quant_trading.decision` | Public FactorSnapshot to non-executing DecisionResult/TradeIntent; owns immutable condition/sizing trace and Decision-history meaning; no production policy | `TradingDecisionPolicy`, `TradingDecisionEngine`, `DecisionResult`, `TradeIntent`, trace/history contracts and query service | stdlib, Factor public models/interfaces | Orchestration, Persistence adapters or independent decision caller | `docs/modules/trading-decision.md` |
...
| `quant_trading.target_position` | Explicit bounded finite-knot target-level definitions and exact manual scalar/USD previews with structured traces | `TargetPositionService`, `TargetPositionEngine`, `TargetPositionStore`, `TargetPositionQueryService`, typed schema-v1 contracts | stdlib, shared errors, neutral Run History contracts | SQLite adapter and Algorithm Control owner page only; no runtime consumer | `docs/modules/target-position.md` |
| `quant_trading.algorithm_control` | Registry/ParameterSchema component management, immutable Factor/Decision versions, safe previews/audit, typed inspectors, exact Factor chart/export and Capital/Asset State owner pages | `AlgorithmControlController`, `AlgorithmControlPanel`, history panels, `CapitalAllocationPanel`, `AssetStatePanel`, chart/export services, `python -m quant_trading.algorithm_control` | stdlib, PySide6, Plotly, shared renderer, public Factor/Decision/Risk/Run/Capital/Asset State contracts | Desktop user; local preview and explicit research management | `docs/modules/algorithm-control-gui.md` |
...
## Verified module status inventory (2026-07-20)
...
| Trading Decision Layer | `PARTIALLY_IMPLEMENTED` | 合同/Engine/受限规则和条件/Sizing trace已验证；无生产Policy/仓位/订单 |
...
| Algorithm Control GUI | `IMPLEMENTED_VERIFIED` | 本地版本/预览/审计和只读Run/Factor/Decision历史GUI及测试；所有组件默认禁用 |

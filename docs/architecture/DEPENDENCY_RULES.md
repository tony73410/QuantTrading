...
- Generic Decision may read Asset/Market Factor contracts and read-only Portfolio sizing context. The approved specialized Target Adjustment Decision may read only source-neutral exact Phase 5C link/target DTOs plus neutral Run identity; neither may modify accounting.
...
- `quant_trading.persistence` may implement public Store/query Protocols and use neutral Run History plus public Market/Factor/Decision/Risk/Capital result/history models, but it must not own formulas/rules, reconstruct missing domain evidence, contain GUI, call Providers, mutate Capital/Accounting or access execution. It must transactionally revalidate Phase 5D source/link/result/Run identity, Phase 6A source/safety/locked-rule/no-approval evidence, Phase 6B exact current-cap/source/formula/non-expansion evidence, Phase 6C exact current-floor/Phase6B/Target-basis/formula/non-expansion evidence, and Phase 6D exact Phase6C/current-plan/latest-snapshot/conservation/bucket/formula/non-reservation evidence. Pure business modules must not import concrete SQLite adapters.
- Algorithm Control may consume injected Run/Factor/Decision/owner-domain query contracts. GUI code must not execute SQL, calculate Factor/Decision values, sign/absolute-value arithmetic or reconstruct historical outcomes; local preview orchestration persists evidence through injected Store contracts.
...
- The Phase 5D coordinator must require one explicit accepted Phase 5C link ID, resolve its exact linked Target result and source evidence through public queries, copy the persisted signed target difference without recomputation/default/fallback, delegate exact sign mapping to the Decision-owned service and persist typed relationships through injected ports. It must not invoke Risk, Backtesting, Accounting or Execution.
- Phase 5D uses specialized `TargetAdjustmentDecisionResult`/`TargetAdjustmentTradeIntent` types. Current Risk and every Backtesting/Accounting/Execution boundary must reject or be unable to accept them; implementation existence and `NO_EXECUTION` history never constitute Risk approval.
- Phase 6A is the only approved Risk consumer of `TargetAdjustmentTradeIntent`, through a source-neutral DTO resolved by orchestration. It must not cast to generic `TradeIntent`, invoke the generic Risk engine or emit generic `RiskDecision`/`RiskApprovedTradeIntent`.
- Phase 6B may consume only one explicit Phase 6A manual-review result through public query contracts. Its Risk-owned rule may preserve/reduce/block the original direction, but a positive candidate is still unapproved; no generic Risk-approved type or downstream trading consumer may be constructed.
- Phase 6C may consume only one explicit positive Phase 6B manual-review result, one explicit current same-symbol floor version and the exact linked Target research basis through public query contracts. It preserves immutable order-1 evidence and applies only the Risk-owned order-2 cash-floor rule. Hypothetical residual cash is never Capital Allocation, Accounting or broker cash; a positive candidate remains unapproved and no downstream trading consumer may be constructed.
- Phase 6D may consume only one explicit positive Phase 6C manual-review result and one explicitly selected Phase 3A `RESEARCH_INPUT` plan with its exact latest conserved snapshot through public read-only query contracts. Risk receives copied source-neutral evidence and must not import or mutate Capital Allocation. It preserves immutable order-1/order-2 references, applies only the Risk-owned order-3 asset-cash limit, stores `research_cash_reserved=false`, and cannot create factual cash, approval or a downstream trading consumer.
- The specialized Risk service owns exactly three locked ordered structural rules. Valid safe evidence always stops at `MANUAL_REVIEW_REQUIRED`; unsafe execution metadata blocks; approved notional and approved intent identities are structurally absent. Numerical Risk, account truth and downstream consumers remain separate approval boundaries.

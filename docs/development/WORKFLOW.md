...
阅读根 `AGENTS.md`、文档索引、项目状态、相关模块/ADR、代码和最近相关编辑记录；运行 `git status`（若已初始化），识别并保护用户未提交修改。
...
列出涉及文件/模块、接口、依赖、配置、数据和交易安全影响；判断是否触发审批。
...
运行最相关测试及必要的格式、类型、静态分析、接口兼容和 diff 检查。失败必须如实区分新引入与既有问题。

...
检查项目包括 Python、依赖、配置来源、运行目录写权限、SQLite 只读连接、逻辑 Schema 版本、`PRAGMA quick_check`、凭据是否完整和交易安全默认值。逐项结果为`PASS / WARNING / FAIL / SKIPPED`，末行使用统一合同汇总`SYSTEM_HEALTH`。默认网络检查被跳过时状态为`UNKNOWN`并保持自动执行不允许。

...
| `QT-CONTRACT` | 公共合同 | 输入/输出或模块合同不兼容 |
| `QT-INTEGRITY` | 验证/健康 | 验证器失败或系统完整性检查 |
...
检查项目包括 Python、依赖、配置来源、运行目录写权限、SQLite 只读连接、真实迁移版本、当前版本要求的完整逻辑表集合、`PRAGMA quick_check`、`PRAGMA foreign_key_check`、凭据是否完整和交易安全默认值。Schema版本/迁移历史/必需表不一致会明确`FAIL`，诊断不会自动迁移或修复数据库。逐项结果为`PASS / WARNING / FAIL / SKIPPED`，末行使用统一合同汇总`SYSTEM_HEALTH`。默认网络检查被跳过时状态为`UNKNOWN`并保持自动执行不允许。
